...
Purpose:	Use MPI to implement a parallel version of the trapezoidal rule.
			This version use collective communications and MPI derived datatypes to distribute the input data and compute the global sum.
...
Algorithm:
			...
			3a. Each process != 0 send its integral to 0.
...
Purpose:		Build a derived datatype so that the three input values can be sent in a single message
...
/*
Function:	Get_input
Purpose:	Get the user input: the left adn right endpoints and the number of trapezoids
...
/*
Function:	Trap
Purpose:	Serial function for estimating a definete integral using the trapezoidal rule
Input args: left_endpt
			right_endpt
			trap_count
			base_len
Return val:	Trapezoidal rule estimate of integral from left_endpt to right_endpt using trap_count trapezoids
*/
...
/*
Function:		f
Purpose:		Compute value of function to be integrad
Input args:		x
*/
double f(double x) {
	return x*x;
