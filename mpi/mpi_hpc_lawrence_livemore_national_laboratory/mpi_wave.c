...
Description:
	...
	A vibrating string is decomposed into points. Each processor is responsible for updating the mplitude of a number of points over
	time. At each iteration, each processor exchanges boundary points with nearest neighbors. This version uses low level sends and receives
*/
...
void init_line(void) {
	...
	for (i = 0, k = 0; i < numtasks; i++) {
		...
		if (taskid == i) {
			first = k + 1;
			npoints = npts;
			printf("task=%3d first point=%5d npoints=%d\n", taskid, first, npts);
			for (j = 1; j <= npts; j++, k++) {
...
int main (int argc, char *argv[]) {
	...
	*/
	...
	// Update values along the line for nsstep time steps
	update(left, right);
